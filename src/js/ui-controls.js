import { prevPage, nextPage, goToPage, setZoom, searchInDocument, currentPageNumber, totalPages } from './pdf-viewer.js';
...
function setupZoomControls() {
  ...
  zoomInButton.addEventListener('click', () => {
    ...
    if (currentValue === 'auto') {
      ...
      const nextIndex = Math.min(zoomSelect.options.length - 2, zoomSelect.selectedIndex + 1);
...
function setupSidebarControls() {
  ...
  sidebarToggle.addEventListener('click', () => {
    ...
    document.querySelector('.pdf-viewer').classList.toggle('sidebar-closed', !isSidebarOpen);
  });
